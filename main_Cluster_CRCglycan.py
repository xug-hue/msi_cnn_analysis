...
def load_info_imzml():
    ...
    imzml_path = "MSI/data/e2.imzML"
    # ibd_path = "MSI/data/e2.ibd"
...
def load_data_uint16_imzml():
    imzml_path = "MSI/data/a2.imzML"
    # ibd_path = "MSI/data/a2.ibd"
    ...
    with imzmlp.ImzMLParser(imzml_path) as parser:
        ...
        intdata = np.zeros((len(coordinatesNPArray), len(ints0)), dtype=np.float)
    ...
    return intdata.astype(np.uint32), num_feature
...
def load_data_imzml():
    ...
    with imzmlp.ImzMLParser(imzml_path) as parser:
        ...
        # 所有谱数据坐标信息，只保留两个维度
        coordinatesNPArray = np.array(parser.coordinates)[:, 0:2]
        # print(coordinatesNPArray.shape)
        # e2数据特殊处理,删除文章中图片没有的数据，左右两块
        list_delete = []
        for i in range(len(coordinatesNPArray)):
            if imzml_path.endswith('e2.imzML'):
        ...
        # 对所有数据遍历，做处理
        dataindex = 0
        for pixelIndex in range(len(coordinatesNPArray)+len(list_delete)):
            # 读出指定像素的质谱数据,若有不需要的数据，则不保存
            if pixelIndex not in list_delete:
                mzs, ints = parser.getspectrum(pixelIndex)
                intdata[dataindex] = normalize(ints)
                dataindex += 1
                if len(mzs) < minlen:
                    minlen = len(mzs)
                if len(mzs) > maxlen:
...
def save_data_csv(data):
    csvdata_path = "MSI/data/e2/MSINorm-all-CRC.csv"
...
if __name__ == "__main__":
    ...
    for i in range(numoftimes):
        ...
        plt.savefig("MSI/result/CRCglycan/clustering/e2/grey" + str(i) + "_" + cluster_method + "_" + str(n_clusters) + ".jpg",
                    dpi=300,
                    bbox_inches='tight',
                    pad_inches=-0.01)
        # 彩色图
        ...
        plt.savefig("MSI/result/CRCglycan/clustering/e2/color" + str(i) + "_" + cluster_method + "_" + str(n_clusters) + ".jpg",
                    dpi=300,
                    bbox_inches='tight',
                    pad_inches=-0.01)
        # plt.show()
    ...
    # 保存聚类结果到文件
    with open("MSI/result/CRCglycan/clustering/e2/clusterbiaozhu.csv", 'w', encoding='utf-8', newline="") as f:
        for i in range(len(y_Predicted_nparray)):
            csv_write = csv.writer(f)
            csv_write.writerow(y_Predicted_nparray[i])

    with open("MSI/result/CRCglycan/clustering/e2/coordinateXY.csv", 'w', encoding='utf-8', newline="") as f:
